...
public class ImageServiceSecurity extends BaseServiceSecurity implements ImageService {
    ...
    public void saveDetails(final ImageDetailsDTO details) {
	    ...
	    // Java 8 doesn't allow this: owner = EntityHelper.reference(test ? (Class<Ad>)Ad.class : (Class<Member>)Member.class, details.getOwner().getId());

            if(details.getNature() == Image.Nature.AD) {
                owner = EntityHelper.reference(Ad.class, details.getOwner().getId());
            } else {
		owner = EntityHelper.reference(Member.class, details.getOwner().getId());
            }
        }

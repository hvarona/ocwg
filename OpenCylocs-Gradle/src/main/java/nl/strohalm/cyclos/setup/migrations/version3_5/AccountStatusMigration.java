...
public class AccountStatusMigration implements UntraceableMigration {
    ...
    public void execute(final JDBCWrapper jdbc) throws SQLException {
        ...
        final String transferQuery = "select id, date, amount, from_account_id, to_account_id, parent_id, account_fee_log_id from transfers where from_account_id = ? or to_account_id = ? order by date";
        ...
        final String statusInsert = "insert into account_status (subclass, account_id, date, root_credits_count, root_credits_amount, root_debits_count, root_debits_amount, nested_credits_count, nested_credits_amount, nested_debits_count, nested_debits_amount, credit_limit, upper_credit_limit, transfer_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        while (rsAccounts.next()) {
            ...
            while (rsTransfers.next()) {
                ...
                final BigDecimal amount = settings.round(rsTransfers.getBigDecimal("amount"));
                ...
                final boolean isDebit = accountId == fromAccountId;
                if (isDebit) {
                    if (isRoot) {
                ...
                jdbc.execute(statusInsert, subclass, accountId, date, rootCreditsCount, rootCreditsAmount, rootDebitsCount, rootDebitsAmount, nestedCreditsCount, nestedCreditsAmount, nestedDebitsCount, nestedDebitsAmount, creditLimit, upperCreditLimit, transferId);
            }

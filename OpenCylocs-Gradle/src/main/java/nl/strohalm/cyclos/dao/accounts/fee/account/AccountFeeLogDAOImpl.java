...
public class AccountFeeLogDAOImpl extends BaseDAOImpl<AccountFeeLog> implements AccountFeeLogDAO {
    ...
    public Iterator<MemberAccount> iterateOverAccountsWithAccountFeeChargesFor(final AccountFeeLog log) {
        ...
        final StringBuilder hql = DatabaseHelper.getInitialQuery(MemberAccount.class, "a");
    ...
    public List<AccountFeeLog> search(final AccountFeeLogQuery query) {
        ...
        final StringBuilder hql = DatabaseHelper.getInitialQuery(getEntityType(), "l", fetch);
        DatabaseHelper.addInParameterToQuery(hql, namedParameters, "l.accountFee", query.getAccountFees());
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "l.accountFee.enabled", query.getAccountFeeEnabled());
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "l.accountFee.accountType", query.getAccountType());
        if (query.getPeriodStartAt() != null) {
            DatabaseHelper.addPeriodParameterToQuery(hql, namedParameters, "l.period.begin", Period.day(query.getPeriodStartAt()));
        }
        DatabaseHelper.appendOrder(hql, "l.date desc, l.id desc");

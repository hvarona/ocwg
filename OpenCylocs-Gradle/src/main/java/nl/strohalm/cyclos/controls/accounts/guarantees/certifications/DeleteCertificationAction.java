...
public class DeleteCertificationAction extends BaseAction {
    protected CertificationService certificationService;

    @Override
    public ActionForward executeAction(final ActionContext context) throws Exception {
        final DeleteCertificationForm form = context.getForm();
        if (form.getCertificationId() <= 0) {
            throw new ValidationException();
        }
        certificationService.remove(form.getCertificationId());
        context.sendMessage("certification.removed");

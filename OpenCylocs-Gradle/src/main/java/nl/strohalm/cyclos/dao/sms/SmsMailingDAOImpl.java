...
public class SmsMailingDAOImpl extends BaseDAOImpl<SmsMailing> implements SmsMailingDAO {
    ...
    public void assignUsersToSend(final SmsMailing smsMailing, final MemberCustomField smsCustomField) {
        ...
        Query qS = DatabaseUtil.getCurrentEntityManager().createQuery((sql.toString()));
        int i = 0;
        for (Object param : params) {
            qS.setParameter(i, param);
            i++;
        }
        qS.executeUpdate();
        ...
        DatabaseUtil.getCurrentEntityManager().createQuery(updateSentSQL).setParameter(0, smsMailing.getId()).executeUpdate();
    }

...
public class ListContactsActionPolicy extends AbstractActionPolicy {
    private final static ListContactsActionPolicy INSTANCE = new ListContactsActionPolicy();

    public static ActionPolicy instance() {
        return INSTANCE;
    }

    private ListContactsActionPolicy() {
    }
    ...
    protected boolean doCheck(final ActionDescriptor descriptor) {
        if (LoggedUser.isMember()) {
            ...
            return hasPermission(OperatorPermission.CONTACTS_VIEW);
        } else {
            return false;
        }
    }

...
public class DocumentDAOImpl extends BaseDAOImpl<Document> implements DocumentDAO {
    ...
    public List<Document> search(final DocumentQuery query) throws DaoException {
        ...
        final StringBuilder hql = DatabaseHelper.getInitialQuery(getEntityType(), "doc", query.getFetch());
        ...
        // Document id
        if (query.getId() != null) {
            DatabaseHelper.addParameterToQuery(hql, namedParameters, "doc.id", query.getId());
        }
        ...
        DatabaseHelper.appendOrder(hql, "doc.name");

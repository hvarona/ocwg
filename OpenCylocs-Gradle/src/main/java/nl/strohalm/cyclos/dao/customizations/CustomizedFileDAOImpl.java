...
public class CustomizedFileDAOImpl extends BaseDAOImpl<CustomizedFile> implements CustomizedFileDAO {
    ...
    public CustomizedFile load(final Type type, final String name, final Relationship... fetch) throws DaoException {
        final StringBuilder hql = DatabaseHelper.getInitialQuery(getEntityType(), "f", fetch == null ? null : Arrays.asList(fetch));
        final Map<String, Object> namedParameters = new HashMap<String, Object>();
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "f.type", type);
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "f.name", name);
    ...
    public List<CustomizedFile> search(final CustomizedFileQuery query) {
        final StringBuilder hql = DatabaseHelper.getInitialQuery(getEntityType(), "f", query.getFetch());
        final Map<String, Object> namedParameters = new HashMap<String, Object>();
        if (!query.isAll()) {
            DatabaseHelper.addParameterToQuery(hql, namedParameters, "f.type", query.getType());
            final Group group = query.getGroup();
            if (group == null) {
                hql.append(" and f.group is null");
            } else {
                DatabaseHelper.addParameterToQuery(hql, namedParameters, "f.group", group);
            }
            final GroupFilter groupFilter = query.getGroupFilter();
            if (groupFilter == null) {
                hql.append(" and f.groupFilter is null");
            } else {
                DatabaseHelper.addParameterToQuery(hql, namedParameters, "f.groupFilter", groupFilter);
            }
        }
        DatabaseHelper.appendOrder(hql, "f.type", "f.name");

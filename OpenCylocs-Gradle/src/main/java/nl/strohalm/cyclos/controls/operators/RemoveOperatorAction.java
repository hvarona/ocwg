 ...
 * @author jefferson
 */
public class RemoveOperatorAction extends RemoveElementAction {

    @Override
    protected ActionForward doRemove(final long id, final ActionContext context) {
        try {
            elementService.remove(id);
            context.sendMessage("changeGroup.operator.permanentlyRemovedMessage");
            return context.getSuccessForward();
        } catch (final Exception e) {
            return context.sendError("changeGroup.error.remove.activeOperator");
        }
    }

...
public class MessageDAOImpl extends BaseDAOImpl<Message> implements MessageDAO {
    ...
    public void assignPendingToSendByBroker(final Message message, final Member broker) {
        ...
        getSession().createQuery(sql.toString()).setParameter(0, params.toArray()).executeUpdate();
    }
    ...
    public void removeMessagesOnTrashBefore(final Calendar limit) {
        ...
        getSession().createQuery(deleteMessagesGroups).setParameter(0, limit).executeUpdate();

        // Then, delete the messages
        final String deleteMessages = "delete from messages where removed_at < ?";
        getSession().createQuery(deleteMessages).setParameter(0, limit).executeUpdate();

...
public class DoPaymentDTO extends DataObject implements CustomFieldsContainer<PaymentCustomField, PaymentCustomFieldValue> {
     ...
     * Optional.
     *
     * @returns the data set by the client making a payment that will be
     * attached to the transfer and sent back when a notification related to
     * this payment is issued by Cyclos (likely to the same client).<br>
     * It depends on the client side then there is no guarantee of uniqueness
     * between different clients.<br>
     * Note: <b>It has nothing to do with the traceNumber field in the parent
     * class (used to tag transactions and query by these value).</b>
     */

...
public class RemarkDAOImpl extends BaseDAOImpl<Remark> implements RemarkDAO {
    ...
    public List<? extends Remark> search(final RemarkQuery query) {
        ...
        final StringBuilder hql = DatabaseHelper.getInitialQuery(nature.getType(), "r", fetch);
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "r.writer", query.getWriter());
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "r.subject", query.getSubject());
        DatabaseHelper.addPeriodParameterToQuery(hql, namedParameters, "r.date", query.getPeriod());

        DatabaseHelper.appendOrder(hql, "r.date desc");

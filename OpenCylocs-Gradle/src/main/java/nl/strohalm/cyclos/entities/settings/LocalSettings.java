...
public class LocalSettings extends DataObject {
    ...
    public static enum Language implements StringValuedEnum {
        ...
        CZECH("cs_CZ", "cs"),
        GERMAN("de_DE", "de", new GermanAnalyzer()),
        GREEK("el_GR", "el", new GreekAnalyzer()),
        DUTCH("nl_NL", "nl"),
        PORTUGUESE_BRAZIL("pt_BR", "pt-br", new BrazilianAnalyzer()),
        CHINESE_SIMPLIFIED("zh_CN", "zh-cn"),
        FRENCH("fr_FR", "fr", new FrenchAnalyzer()),
        JAPANESE("ja_JP", "jp"),
        ITALIAN("it_IT", "it"),
        RUSSIAN("ru_RU", "ru");
        ...
        private Language(final String value, final String alternate) {
            this(value, alternate, new StandardAnalyzer());
        }
    ...
    /**
     * Contains parameters for generating a transaction number. The number is
     * composed by 3 parts: a prefix, the padded identifier and a suffix. Both
     * prefix and suffix may contain a date pattern between #, so, for example,
     * if the prefix TN#yy-MM# is processed on january, 2006, the result would
     * be TN06-01. To generate a # character, it must be escaped with \\#
     */
    ...
    /**
     * gets a CalendarConverter which is derived from the normal
     * CalendarConverter, but without the year. To be used for cewolf graphs as
     * Strings along the x-axis, where normal date labels with a year would be
     * to long and make the x-axis too packed with labels.
     *

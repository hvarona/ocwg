...
public class ReferenceHistoryDAOImpl extends BaseDAOImpl<ReferenceHistoryLog> implements ReferenceHistoryDAO {
    ...
    public Map<Level, Integer> countReferencesHistoryByLevel(final Member member, final Collection<MemberGroup> memberGroups, final Period date, final boolean received) {
        ...
        DatabaseHelper.addParameterToQuery(hql, namedParameters, (received ? "rh.to" : "rh.from"), member);
        ...
        if (date != null) {
            ...
            if (begin != null) {
                DatabaseHelper.addParameterToQueryOperator(hql, namedParameters, "rh.period.begin", ">=", begin);
            }
    ...
    public ReferenceHistoryLog getOpenReferenceHistoryLog(final ReferenceHistoryLogQuery query) {
        ...
        final StringBuilder hql = DatabaseHelper.getInitialQuery(getEntityType(), "rhl", fetch);
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "rhl.from", query.getFrom());
        DatabaseHelper.addParameterToQuery(hql, namedParameters, "rhl.to", query.getTo());
